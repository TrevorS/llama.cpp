...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        // when not using events (n_copies == 1), synchronizing the split backend once per split is
        // enough to ensure that the input copies are no longer in use: all uses of the input copies
        // were previously submitted to the split backend, and the copies below write to distinct
        // tensors, either blocking on the host or in submission order on the split backend queue
