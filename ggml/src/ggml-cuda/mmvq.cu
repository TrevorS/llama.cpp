...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    if (has_ids && (GGML_CUDA_MMID_BATCH_INVARIANT || (ggml_cuda_batch_invariant_flags() & GGML_CUDA_BI_MMID) || ncols_dst > 1)) {
    ...
    // [TAG_BATCH_INVARIANT_RUNTIME] rather than detuning width 1 the way the build macro
    // does, launch the width-1 kernel once per column with the same variant choice the solo
    // path makes (the small_k/halve_iters predicates do not depend on ncols_dst). Column
    // arithmetic is column-local end to end, so each launch is bit-identical to a solo
    // decode of that column.
    if (!has_ids && ncols_dst > 1 && (ggml_cuda_batch_invariant_flags() & GGML_CUDA_BI_MMVQ)) {
        // fused mat-vec exists only at ncols_dst == 1, so nothing to offset per column here
        GGML_ASSERT(fusion.gate == nullptr && fusion.x_bias == nullptr);
        for (int j = 0; j < ncols_dst; ++j) {
            mul_mat_vec_q_switch_ncols_dst<type>(
                vx, (const void *) ((const block_q8_1 *) vy + j*stride_col_y), nullptr, fusion,
                dst + j*stride_col_dst,
                ncols_x, nrows_x, 1,
                stride_row_x, stride_col_y, stride_col_dst,
