...
static __global__ void dsv4_hc_weighted_sum_kernel(
    ...
    ggml_cuda_pdl_sync(); // wait on producer (x, w) before the first global read
...
static __global__ void dsv4_hc_post_kernel(
    ...
    ggml_cuda_pdl_sync(); // wait on producers (x, res, post, comb) before the first global read
...
static __global__ void dsv4_hc_sinkhorn_kernel(
    ...
    ggml_cuda_pdl_sync(); // wait on producer (comb) before the first global read
...
void ggml_cuda_op_dsv4_hc_fused(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (mode == GGML_DSV4_HC_MODE_SINKHORN) {
        ...
        const ggml_cuda_kernel_launch_params lp_sink = { dim3(nt), dim3(64), 0, stream };
        ggml_cuda_kernel_launch(dsv4_hc_sinkhorn_kernel, lp_sink,
    ...
    if (mode == GGML_DSV4_HC_MODE_WEIGHTED_SUM) {
        ...
        const ggml_cuda_kernel_launch_params lp_ws = { grid, dim3(block), hc * sizeof(float), stream };
        ggml_cuda_kernel_launch(dsv4_hc_weighted_sum_kernel, lp_ws,
                (float *) dst->data, (const float *) x->data, (const float *) w->data,
                n_embd, hc, nt);
    } else {
        GGML_ASSERT(mode == GGML_DSV4_HC_MODE_POST);
        ...
        const ggml_cuda_kernel_launch_params lp_post = { grid, dim3(block), shmem, stream };
        ggml_cuda_kernel_launch(dsv4_hc_post_kernel, lp_post,
