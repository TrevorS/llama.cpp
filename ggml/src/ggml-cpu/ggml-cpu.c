...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_DSV4_LID_TOPK:
            {
                ggml_compute_forward_dsv4_lid_topk(params, tensor);
            } break;
        case GGML_OP_DSV4_LID_UNION:
            {
                ggml_compute_forward_dsv4_lid_union(params, tensor);
            } break;
        case GGML_OP_DSV4_LID_MEMB:
            {
                ggml_compute_forward_dsv4_lid_memb(params, tensor);
            } break;
        case GGML_OP_DSV4_MOE_GATE_UP:
            {
                ggml_compute_forward_dsv4_moe_gate_up(params, tensor);
            } break;
        case GGML_OP_DSV4_HC_FUSED:
            {
                ggml_compute_forward_dsv4_hc_fused(params, tensor);
            } break;
        case GGML_OP_DSV4_FP4_RT:
            {
                ggml_compute_forward_dsv4_fp4_rt(params, tensor);
            } break;
        case GGML_OP_DSV4_QAT_SET_ROWS:
            {
                ggml_compute_forward_dsv4_qat_set_rows(params, tensor);
            } break;
