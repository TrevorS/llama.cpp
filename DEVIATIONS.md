...
| deep exactness, pinned [shape] at 9000 | **5.709**, 0 flips, where shallow pinned is 0. Identical with the pooled cache off and with two-stage off; the per-head score under pins (`8d3996709`) moves it to 5.482, so the score matmul's kernel was one contributor. Depth scan (per-head score): **1500: 0** (n_kv under the selection width, every cell in), **3000: 2.937**, **6000: 3.322**, 9000: 5.482, single-stage or two-stage alike -- the variance appears exactly when the selection is real and grows with depth; the stable-argsort leg (`LLAMA_QSA_TOPK_SORT=1`) gives **bit-identical** 2.93724 / 5.48222, so the top-k tie-break is cleared too. Every selection-side knob is excluded and the delta needs a real sparse mask: `-fa off` still reads 3.81 at 3000, so the attention kernel is cleared too. **Found**: the indexer q/k projections are BF16, the one weight family the mmvf pin skipped (mat-vec at one column, mma tiles past it). With BF16 pinned to batch 8 (`8f8af0598`) the pinned legs read **0 on every line at 3000 and at 9000**. Serving is unpinned, so this was a research property, not a serving defect. Open, low priority: the dump tool's wide-vs-one-at-a-time schedule (pinned, 3000) still differs from the batch that refills a rolled-back block onward (max 1.38, 0 flips) while the exactness test's rollback scenarios read 0 at 9000; the harness's one-token mode is the first suspect |
