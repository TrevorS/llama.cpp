...
struct test_get_rows : public test_case {
    ...
    std::string vars() override {
        return VARS_TO_STR9(type, n, m, r, be1, be2, v, vs0, type_out);
    }

    test_get_rows(ggml_type type = GGML_TYPE_F32, int n = 10, int m = 5, int r = 3, int be1 = 1, int be2 = 1, bool v = false, bool vs0 = false, ggml_type type_out = GGML_TYPE_F32)
        : type(type), n(n), m(m), r(r), be1(be1), be2(be2), v(v), vs0(vs0), type_out(type_out) {}
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        const bool grad_supported = !vs0 && ggml_is_matrix(in) && ggml_is_vector(rows) && type_out == GGML_TYPE_F32;
        if (grad_supported) {
            ggml_set_param(in);
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    test_cases.emplace_back(new test_get_rows(GGML_TYPE_F16, 256,    5,    4, 1, 1, false, false, GGML_TYPE_F16));
    test_cases.emplace_back(new test_get_rows(GGML_TYPE_F16, 512, 1024, 2304, 2, 1, false, false, GGML_TYPE_F16));
    test_cases.emplace_back(new test_get_rows(GGML_TYPE_F16, 256,    5,    4, 3, 1, true,  false, GGML_TYPE_F16));
    test_cases.emplace_back(new test_get_rows(GGML_TYPE_F32, 256,    5,    4, 1, 1, false, false, GGML_TYPE_F16));
