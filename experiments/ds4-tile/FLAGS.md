...
| `LLAMA_DSV4_LID_FP4_MMA` | OFF (`=1`; Blackwell + packed MXFP4 container only) | register-resident-K block-scaled fp4 tensor-core score kernel: reads 68B block_mxfp4 rows direct (bypasses staging + int8 pre-quant), mma.sync kind::mxf4 m16n8k64, q packed to e2m1 per head, f16 score store → radix | int8 dp4a score path | **MEASURED WIN 2026-07-15**: op 74.3→38.8ms @33280×2048×2048 (−47.8%, score kernel ~71→~35ms — register-resident K converts the L1 bound). Class B numerics (== LID_FP4, on the official e2m1 grid); EXACT combo bit-exact (0.0, packed-direct rescore; oracle pass-1 m-need p100≤4 « m=64). Off-Blackwell → int8 (one-shot warn). **On-model depth A/B 2026-07-15 (both arms CACHE_MXFP4=1, POWER=85): d65k +4.7% (357.4→374.3), d131k +8.6% (263.07→285.57) — advantage grows with depth.** Remaining before default-flip: PPL/coherence gate. |
...
| `LLAMA_DSV4_CSA_TILE_OVF` | OFF (`=N`, e.g. 2048; rounded up to ×256) | EXACT attended set for CSA_TILE: union built with u_cap+N slots, overflow slice rides a 3rd FA leg + 3-way LSE merge (`ggml_dsv4_fa_merge3`) — tiles whose union exceeds u_cap no longer drop their newest cells | historical truncation (tail few % tiles at d65k+ silently drop highest-index cells) | FA_SPLIT path only. Gates 2026-07-16: byte-identity @64k (no-overflow depth), UNION_STATS over=0/26880 @6144, merge3 backend-ops incl. NaN-poison case. Cost @d65k: −2.1% pp full power / −3.8% @POWER=85. Measured union max ~5200 @W16 d131k → 2048 covers; u_cap+N ≥ 8192 is the provable bound. For the exact/audit profile; d131k overflow-active passkey demo pending. Found+fixed: all-masked FA leg returns NaN·(weight 0) — merge now skips zero-weight parts (regression test in backend-ops). |
| `LLAMA_DSV4_KV_BF16_RT` | OFF (`=1`) | round every persisted KV-cache/state row to the bf16 grid at write (raw K ×3, CSA comp, csa/lid/hca state persists; lid cache excluded — QAT is the stronger official numeric) | f16 storage of f32 values (finer than official) | parity/debug instrument for logit-diff vs the official transformers build (official caches are bf16). f16-of-bf16 bit-exact for \|x\|<65504. PPL c4096: 4.2278→4.2240 (−0.09%). Never a serving default. |
