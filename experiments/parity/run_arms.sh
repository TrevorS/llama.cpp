...
N_CTX=$(python3 -c "import json;print(json.load(open('$HERE/meta.json'))['n_ctx'])")
...
ppl() { # ppl <model> <outfile.kld> [extra env as VAR=VAL ...]
  ...
  env LLAMA_DSV4_FUSED_LID=1 "$@" \
    $BIN/llama-perplexity -m $model -f $HERE/corpus.txt -c $N_CTX --chunks $N_CHUNKS \
    --parse-special --no-escape --save-all-logits $out -fa on -ub 2048 -b 2048 --no-mmap \
    > ${out%.kld}.log 2>&1
}
...
for arm in arm2-exact "arm3-iq2" arm4-bf16; do
  ...
  env LLAMA_DSV4_FUSED_LID=1 "${envs[@]}" \
    $BIN/llama-perplexity -m $m -f $HERE/corpus.txt -c $N_CTX --chunks $N_CHUNKS \
    --parse-special --no-escape --kl-divergence-base arm1-fast.kld --kl-divergence \
