...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    ...
    // An MTP-only draft is fed the target's hyper-connection streams, so its input row is
    // n_embd_out() wide rather than n_embd. DeepSeek-V4 gets this by giving the draft its own
    // arch (deepseek4mtp, whose embedding_length is already the wide value); qwen4exp keeps one
    // arch, so declare it here. common/speculative.cpp asserts the two widths match.
    if (hparams.n_layer_nextn > 0 && ml.get_weight("blk.0.hc_attn_norm.weight") == nullptr) {
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // an MTP draft file may name its head the way Unsloth and upstream do, blk.N.nextn.hc_head_*
    // (N = the trunk depth), instead of the target's output_hc_*: accept either
    {
        const int  n_layer_main = (int) (hparams.n_layer_all - hparams.n_layer_nextn);
        const int  head_flags   = hparams.n_layer_nextn > 0 ? TENSOR_NOT_REQUIRED : 0;

        hc_head_norm = create_tensor(tn(LLM_TENSOR_HC_HEAD_NORM, "weight"), { n_embd, hc }, head_flags | TENSOR_ALLOW_RESHAPE);
        hc_head_down = create_tensor(tn(LLM_TENSOR_HC_HEAD_DOWN, "weight"), { hc_dim, hc_lr }, head_flags);
        hc_head_up   = create_tensor(tn(LLM_TENSOR_HC_HEAD_UP,   "weight"), { hc_lr, hc_dim }, head_flags);

        if (hc_head_norm == nullptr) {
            hc_head_norm = create_tensor(tn(LLM_TENSOR_NEXTN_HC_HEAD_NORM, "weight", n_layer_main), { n_embd, hc }, TENSOR_ALLOW_RESHAPE);
            hc_head_down = create_tensor(tn(LLM_TENSOR_NEXTN_HC_HEAD_DOWN, "weight", n_layer_main), { hc_dim, hc_lr }, 0);
            hc_head_up   = create_tensor(tn(LLM_TENSOR_NEXTN_HC_HEAD_UP,   "weight", n_layer_main), { hc_lr, hc_dim }, 0);
        }
    }
    ...
    // The MTP block is one full-attention layer with its own input fusion. It has no PLE and no
    // linear attention, so it is built here rather than in the loop above.
    for (int il = n_layer; il < n_layer + (int) hparams.n_layer_nextn; ++il) {
        ...
        // hnorm covers the whole hyper-connection row, not one stream
...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    // the MTP driver sets masked=false on the target, meaning it wants h_nextn for every row;
    // that defers the last layer's inp_out_ids reduction until after the export below
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // residual, so reducing cur/inject while res_hc stays full width mismatches the reshape.
    ...
    // The MTP draft is fed the trunk's hyper-connection streams, not the collapsed hidden state.
    // res_hc is already row-reduced when masked=true and still full width when masked=false, so
    // the export needs no get_rows of its own. The deferred reduction is chained off `flat` on
    // purpose: an exported view that nothing consumes is a dead node, and the scheduler then
    // assigns it no backend (llama-context.cpp asserts on that when reading h_nextn back).
    {
...
public:
    ...
    void set_input(const llama_ubatch * ubatch) override {
        ...
        mctx->set_input_qsa(cell_blk, bias, win_cells, win_pos, win_blk, blk_cells, blk_pad, k_idxs, ubatch, ratio, blk_bias);
    }
    ...
    bool can_reuse(const llm_graph_params & params) override {
        ...
        // the two-stage decision is part of the shape
    ...
    ggml_tensor * blk_cells = nullptr;   // I32 [ratio*(n_blocks+1), n_stream], two-stage selection only
    ggml_tensor * blk_pad   = nullptr;   // F32 [ratio*(n_blocks+1), n_stream], 0 or -inf per slot
...
// It needs a per-block bias (causal, no alibi), one sequence per stream on plain positions so
...
bool llama_model_qwen4exp::graph::qsa_two_stage(
    ...
    static const int mode = [] {
        const char * e = getenv("LLAMA_QSA_TWO_STAGE");
        return e == nullptr ? 1 : atoi(e);
    }();

    if (mode == 0 || !blk_bias || mctx_hyb == nullptr || mctx_hyb->get_idx() == nullptr) {
        return false;
    }
    ...
    return mctx_hyb->qsa_single_seq(ubatch);
}
...
ggml_tensor * llama_model_qwen4exp::graph::build_qsa_top_k(
    ...
    const bool two_stage = qsa_two_stage(mctx_hyb, ubatch, blk_bias, n_kv);
    ...
    if (it != qsa_inps.end()) {
        ...
        if (two_stage) {
            qsa->blk_cells = ggml_new_tensor_2d(ctx0, GGML_TYPE_I32, r*(n_blocks + 1), n_stream);
            qsa->blk_pad   = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, r*(n_blocks + 1), n_stream);
    ...
    if (two_stage) {
        ...
        ggml_tensor * score_rows = ggml_view_3d(ctx0, score, 1, n_blocks, n_q, score->nb[0], score->nb[1], 0);
        ggml_tensor * sel_score  = ggml_get_rows(ctx0, score_rows, ggml_reshape_2d(ctx0, sel_blk, k_blk, n_q));   // F32 [1, k_blk, n_q]

        // the cells of those blocks, and which slots are pads
        ggml_tensor * idx_tab    = ggml_reshape_2d(ctx0, sel_blk, k_blk*n_tps, n_stream);
        ggml_tensor * cand_cells = ggml_get_rows(ctx0, ggml_reshape_3d(ctx0, inp->blk_cells, r, n_blocks + 1, n_stream), idx_tab);   // I32 [r, k_blk*n_tps, n_stream]
        ggml_tensor * cand_pad   = ggml_get_rows(ctx0, ggml_reshape_3d(ctx0, inp->blk_pad,   r, n_blocks + 1, n_stream), idx_tab);   // F32 [r, k_blk*n_tps, n_stream]

        // stage 2: the cells, scored as block score + mask (-inf where the query cannot see)
        ggml_tensor * mask_rows = ggml_view_3d(ctx0, kq_mask, 1, n_kv, n_q, kq_mask->nb[0], kq_mask->nb[1], 0);
        ggml_tensor * cand_mask = ggml_get_rows(ctx0, mask_rows, ggml_reshape_2d(ctx0, cand_cells, r*k_blk, n_q));   // F32 [1, r*k_blk, n_q]

        ggml_tensor * cand_score = ggml_add(ctx0, ggml_reshape_3d(ctx0, cand_mask, r, k_blk, n_q), ggml_reshape_3d(ctx0, cand_pad, r, k_blk, n_q));
        cand_score = ggml_add(ctx0, cand_score, sel_score);
        ...
        top_k = ggml_cont(ctx0, ggml_get_rows(ctx0, cand_rows, sel_c));   // I32 [1, width, n_q]
    } else {
...
// embedding of the token the target just produced. One full-attention layer, no PLE.
// ref: PR #27739 (closed), sglang qwen4_exp_mtp.py, and deepseek4.cpp for the same block shape.
llama_model_qwen4exp::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    : graph(model, params, true) {
    // a single block also keeps t_h_nextn correct below: the draft context reads it masked, and
    // only a chained head would put non-output tokens in a draft batch (common/speculative.cpp)
    ...
    // fc_embedding(e) + fc_hidden(h) is one projection of the concatenation, fused in conversion
