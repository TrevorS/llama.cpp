...
private:
    ...
    // dspark in-graph draft chain export: (id, conf_logit) pairs, [2*K] floats
    // populated per decode when cparams.dspark_draft_chain is enabled and the
    // graph sets llm_graph_result::t_dspark_meta
    std::vector<float> dspark_meta;
