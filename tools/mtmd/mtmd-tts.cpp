...
// Optimized: precompute inverse of rms to use multiply instead of divide
static void rms_norm(const float * x, const float * weight, float * out, int n, float eps = 1e-6f) {
    float sum_sq = 0.0f;
    for (int i = 0; i < n; ++i) {
        sum_sq += x[i] * x[i];
    }
    // Use 1/rms to replace division with multiplication in the loop
...
// Optimized with cache-friendly loop ordering and specialized m=1 path
static void matmul(const float * a, const float * b, float * out, int m, int k, int n) {
    // Fast path for m=1 (common case in Code Predictor)
    // Uses blocked approach for better cache utilization
    if (m == 1) {
        // For single row: iterate over output columns
    ...
    // General case: use cache-friendly i-l-j ordering
    // First zero the output
    memset(out, 0, m * n * sizeof(float));

    // Accumulate: out[i,j] += a[i,l] * b[j,l]
    // This ordering keeps a[i,l] in register while iterating over j
    for (int i = 0; i < m; ++i) {
        for (int l = 0; l < k; ++l) {
            float a_val = a[i * k + l];
            float * out_row = out + i * n;
            for (int j = 0; j < n; ++j) {
                out_row[j] += a_val * b[j * k + l];
            }
        }
    }
}
...
static int sample_token(const float * logits, int n_vocab, float temperature, int top_k,
    ...
    // Use unordered_set for O(1) lookup instead of std::set's O(log n)
    std::unordered_set<int> recent_set(recent_tokens.begin(), recent_tokens.end());
...
static bool run_code_predictor_inline(
    ...
    // Pre-cache LM heads and codec embeddings to avoid repeated tensor copies in the loop
    // This provides significant performance improvement for Code Predictor inference
    ...
    for (int cb = 0; cb < CP_N_CODEBOOKS; ++cb) {
        if (cb < (int)model->talker_cp_lm_head.size()) {
            if (!copy_tensor_to_cpu(model->talker_cp_lm_head[cb], lm_head_cache[cb])) {
                fprintf(stderr, "Error: Failed to pre-cache LM head %d\n", cb);
                return false;
            }
        }
        if (cb < (int)model->talker_cp_codec_embd.size()) {
            if (!copy_tensor_to_cpu(model->talker_cp_codec_embd[cb], codec_embd_cache[cb])) {
                fprintf(stderr, "Error: Failed to pre-cache codec embedding %d\n", cb);
                return false;
            }
        }
    }

    // Also cache the last embedding table for last_residual_hidden
    std::vector<float> last_embd_cache;
    if (!model->talker_cp_codec_embd.empty()) {
        int last_table_idx = (int)model->talker_cp_codec_embd.size() - 1;
        if (!copy_tensor_to_cpu(model->talker_cp_codec_embd[last_table_idx], last_embd_cache)) {
            fprintf(stderr, "Warning: Failed to pre-cache last codec embedding table\n");
        }
    }
    ...
    // Uses pre-cached LM heads and codec embeddings for better performance
    std::vector<float> logits(CP_VOCAB);
    for (int cb = 0; cb < CP_N_CODEBOOKS; ++cb) {
        int pos = cb + 2;

        // Apply output norm and LM head (using cached weights)
        ...
        // Get embedding for this token (using cached embeddings)
        if (cb < (int)codec_embd_cache.size() && !codec_embd_cache[cb].empty()) {
            const std::vector<float> & embd_data = codec_embd_cache[cb];
    ...
    // Add last_residual_hidden from cached final embedding table
    if (!codebook_tokens.empty() && !last_embd_cache.empty()) {
        int last_token = codebook_tokens.back();
        std::vector<float> last_residual_hidden(CP_N_EMBD);
        if (last_token >= 0 && last_token < CP_VOCAB) {
            for (int i = 0; i < CP_N_EMBD; ++i) {
                last_residual_hidden[i] = last_embd_cache[last_token * CP_N_EMBD + i];
            }
        }
