    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        if (fuse_ingest) {
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                n_rows = std::min(n_rows, (int32_t) (dp.pos0 - pos0));
                ...
                if (pos0 + j0 != pos_max + 1 || pos0 + n_rows != dp.pos0) {
                    ...
                    SPC_DBG("abandoning ingest fusion (seq_id=%d, pos0=%d, n=%d, j0=%d, dft pos_max=%d, n_past=%d)\n",
                            (int) seq_id, (int) pos0, n_rows, j0, (int) pos_max, (int) dp.pos0);
                    drop_pending(seq_id);
                    continue;
                }
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            keep_pos[seq_id] = fuse_ingest ? dp.pos0 : -1;
